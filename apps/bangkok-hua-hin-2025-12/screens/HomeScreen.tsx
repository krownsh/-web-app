...
export const HomeScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const weatherRes = await fetch(
                    `https://api.open-meteo.com/v1/forecast?latitude=${trip.weather_lat}&longitude=${trip.weather_lng}&hourly=temperature_2m,precipitation_probability,weathercode&current_weather=true&timezone=${encodeURIComponent(trip.timezone || 'UTC')}&forecast_days=1`
                );
        ...
        fetchData();
    }, [trip?.id, trip?.currency, trip?.weather_lat, trip?.weather_lng, trip?.timezone, trip?.exchange_rate]);
    ...
    const handleAddMustBuy = async () => {
        ...
        const success = await SupabaseService.addRecord('thaiwomao_must_buys', payload);
        if (success) {
    ...
    const handleDeleteMustBuy = async (id: string, e?: any) => {
        ...
        if (id.length > 20) { // UUID heuristic
            try {
                await SupabaseService.deleteRecord('thaiwomao_must_buys', id);
            } catch (e) {
        ...
        <div className="flex-1 h-full overflow-y-auto no-scrollbar relative pb-32">
                ...
                <div className="absolute inset-0 bg-white/10 backdrop-blur-3xl border-b border-white/10 flex flex-col items-center justify-center pb-8">
                    ...
                    <div className="mt-6 flex items-center gap-3 bg-white/10 backdrop-blur-xl px-8 py-2.5 rounded-full border border-white/20 shadow-lg">
                        <span className="text-xs text-white/80 font-medium tracking-widest uppercase">
                            {tripState.type === 'countdown' ? `距離出發還有 ${tripState.value} 天` : (days.find((d) => d.day_key === currentDayKey) ? `${days.find((d) => d.day_key === currentDayKey)?.month_label} ${days.find((d) => d.day_key === currentDayKey)?.date_num} · ${days.find((d) => d.day_key === currentDayKey)?.weekday_label}` : trip?.title)}
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div className="glass-panel p-5 rounded-[2rem] shadow-sm border border-white/60 relative overflow-hidden">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-bold text-zen-text-light tracking-widest uppercase">{placeLabel}</span>
                            </div>
                    ...
                    <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 -mx-1 px-1">
                        {weatherData.hourly.length > 0 ? (
                            ...
                            <div className="w-full text-center text-xs text-zen-text-light py-4">正在獲取{placeLabel}天氣…</div>
                        )}
                    </div>
                </div>
                ...
                <div className={`glass-panel p-5 rounded-[2rem] shadow-sm border border-white/60 transition-all duration-300 ${isEditing ? 'ring-4 ring-zen-moss/20 bg-white/90' : 'bg-white/60'}`}>
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-zen-text-light">即時匯率: 1 {money.code} ≈ {Number(liveRate).toFixed(2)} TWD</p>
                            </div>
                        </div>
                        {isEditing && (
                    ...
                    <div className="flex items-center gap-3 bg-white/50 rounded-2xl p-1 border border-zen-moss/20">
                        ...
                        <div className="flex-1 flex flex-col items-center p-3 rounded-xl bg-white shadow-sm border border-white">
                            <span className="text-[10px] text-zen-text-light font-bold uppercase tracking-wider mb-1">{money.name} ({money.code})</span>
                            <input
                                type="number"
                                value={foreignAmount}
                                placeholder="0"
            ...
            <div className="flex flex-col gap-6 px-5 relative z-20 pt-2">
                ...
                <div>
                    ...
                    <div className="bg-white rounded-organic overflow-hidden h-[340px] flex flex-col relative border border-zen-rock/30">
                        {isItineraryLoading && (
                        ...
                        <div className="w-full h-[35%] flex items-center gap-3 p-3 bg-white">
                            {activeItem?.image && (
                                <img src={activeItem.image} alt="" className="h-full w-24 object-cover rounded-xl shrink-0 bg-zen-bg" />
                            )}
                            <div className="flex flex-col items-center text-center gap-0.5 flex-1 min-w-0">
                                <span className="text-[8px] font-bold text-zen-text-light uppercase tracking-widest whitespace-nowrap">集合地點</span>
                                ...
                                <button
                                    ...
                                    className="mt-1 text-[9px] font-medium text-zen-moss border border-zen-moss/30 px-2 py-0.5 rounded-full active:bg-zen-moss active:text-white transition-colors whitespace-nowrap"
                                >
                                    設定
                                </button>
                            </div>
