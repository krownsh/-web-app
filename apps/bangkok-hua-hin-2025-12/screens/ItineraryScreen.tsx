...
const ItineraryScreen: React.FC = () => {
    ...
    if (!selectedItem) {
        return (
            <div className="flex flex-col h-full bg-zen-bg font-display items-center justify-center gap-3 px-6">
                <p className="text-sm text-zen-text-light">{itemsLoading ? '載入中…' : '這團還沒有行程項目。'}</p>
                <Link to="/discovery" className="text-xs text-zen-moss">回行程</Link>
            </div>
        );
    }

    return (
        <div className="flex flex-col h-full bg-zen-bg font-display overflow-hidden">
            {/* Header / Top Bar */}
            <header className="shrink-0 bg-white/80 backdrop-blur-md border-b border-zen-rock/10 pt-4 pb-2 shadow-sm z-50">
                ...
                >
                    {allItems.map((item: any, index: number) => {
                        ...
                        const isNewDay = index === 0 || allItems[index - 1].dayLabel !== item.dayLabel;
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-6 pb-32">
                ...
                <div className="text-center mb-8">
                    {imageForItem(selectedItem.title, selectedItem.image || selectedItem.image_url) && (
                        <img
                            src={imageForItem(selectedItem.title, selectedItem.image || selectedItem.image_url)}
                            alt={currentLocationName}
                            className="w-full aspect-square max-h-64 object-cover rounded-2xl mb-5"
                        />
                    )}
                    <h2 className="text-2xl font-bold text-zen-text tracking-wide mb-2 leading-snug">{currentLocationName}</h2>
                    <p className="text-sm text-zen-text-light mb-3 leading-relaxed break-all whitespace-pre-wrap px-4">{selectedItem.desc}</p>
