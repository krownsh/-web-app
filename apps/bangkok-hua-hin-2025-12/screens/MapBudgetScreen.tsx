...
const MapBudgetScreen: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        if (editingId !== null) {
            ...
            const success = await SupabaseService.updateRecord('thaiwomao_budget_records', editingId.toString(), recordData);
            if (success) {
                setRecords(records.map((r: any) => r.id === editingId ? { ...r, ...recordData } : r));
            }
        } else {
            const success = await SupabaseService.addRecord('thaiwomao_budget_records', { ...recordData, trip_id: trip!.id, owner_id: myUserId });
            if (success) {
