...
const DiscoveryScreen: React.FC = () => {
    ...
    const [fullItinerary, setFullItinerary] = useState<any>({});
    ...
    useEffect(() => {
        ...
        syncCurrent();
    }, [fullItinerary, trip?.id]);

    useEffect(() => {
        if (!trip?.id) return;
        const loadFromSupabase = async () => {
            try {
                const cloudItinerary = await SupabaseService.getItinerary(trip.id);
                if (cloudItinerary && cloudItinerary.length > 0) {
                    setFullItinerary(groupItineraryByDay(days, cloudItinerary));
                }
            } catch (error) {
    ...
    const mapCenter: [number, number] = selectedLocation
        ...
        : [trip?.weather_lat || 14.5547, trip?.weather_lng || 121.0244];
    ...
    if (!trip) {
        return (
            <div className="flex h-full items-center justify-center bg-zen-bg text-zen-text-light text-sm">
                載入行程中…
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-full bg-zen-bg overflow-hidden relative page-enter">
                ...
                >
                    ...
                    {polylinePositions.length > 1 && (
                        <Polyline
                            ...
                            pathOptions={{ color: '#1c3d4a', weight: 3, dashArray: '5, 10', opacity: 0.7 }}
                        />
                    )}
            ...
            <div className="absolute top-0 left-0 right-0 z-20 p-4 pt-8 flex flex-col gap-4 pointer-events-none">
                <div className="flex justify-between items-center">
                    <MotionLink to="/" className="pointer-events-auto flex size-10 items-center justify-center rounded-full bg-white border border-zen-rock shadow-sm text-zen-text">
                ...
                <div className="pointer-events-auto flex justify-center">
                    <div className="flex bg-white/90 backdrop-blur-md rounded-[20px] px-6 py-2 shadow-mist border border-white/50 gap-6">
                        {Object.keys(fullItinerary).map((day) => {
                            const isActive = activeDay === day;
                            const dayData = fullItinerary[day] || {};
                            return (
                                <button
                                    key={day}
                                    onClick={() => setActiveDay(day)}
                        className={`flex flex-col items-center justify-center gap-0.5 transition-transform duration-300 ${isActive ? 'scale-110' : 'active:opacity-70'}`}
                                >
                                    ...
                                    <div className={`mt-0.5 w-1 h-1 rounded-full transition-all duration-300 ${isActive ? 'bg-zen-moss shadow-[0_0_8px_rgba(28,61,74,0.45)]' : 'bg-transparent'}`}></div>
                                </button>
                            );
                        })}
                    </div>
                            ...
                            {dayItems.map((item: any, idx: number) => {
                                ...
                                return (
                                    <div
                                        ...
                                        onClick={() => {
                                            if (item.lat && item.lng) setSelectedItemId(item.id);
                                        }}
                                    >
                                            ...
                                            <div className={`flex bg-white border rounded-xl shadow-sm overflow-hidden transition-all duration-300 ${isSelected ? 'border-zen-moss ring-1 ring-zen-moss/20 shadow-md' : 'border-white/60'}`}>
                                                ...
                                                <div className="flex-1 flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-zen-rock/40">
                                                    ...
                                                    <div className="flex-1 p-3 flex items-start gap-2">
                                                        {item.image ? (
                                                            <img src={item.image} alt="" className="size-12 rounded-lg object-cover shrink-0 bg-zen-bg" />
                                                        ) : (
                                                ...
                                                <div className="flex flex-col w-12 border-l border-zen-rock/20 bg-zen-mist/5 divide-y divide-zen-rock/20">
                                                    ...
                                                    <MotionLink
                                                        ...
                                                        className="flex-1 flex items-center justify-center text-zen-rock active:bg-zen-mist active:text-cta"
                                                    >
